...
let data: IImage;
export let previewImageUrl: string | ArrayBuffer | null | undefined = '';
...
const downloadImage = async ( ) => {
    if(previewImageUrl){
        try {
            const response = await fetch(previewImageUrl.toString());
...
const redirectToDetail = () => {
    const newTab = window.open(`${import.meta.env.VITE_BASE_URL}/${data?.slug}`, '_blank');
    newTab?.focus(); 
}
...
<div class="w-2/5">
    <div class="w-full flex items-center justify-between gap-4">
        <div>
            <Button on:click={resetImage} variant="danger">
                Reset
            </Button>
        </div>
        <div class="flex items-center justify-end gap-4">
            <Button on:click={redirectToDetail}>
